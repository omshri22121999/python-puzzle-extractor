...
class PositionTest:
    def __init__(
        ...
        best_evaluation: chess.engine.Score = None,
        next_best_evaluvation: chess.engine.Score = None,
        depth: int = 17,
    ):
    ...
    def move_list(self):
        if (
            self.next_position is None
            or self.next_position.ambiguous()
            or self.next_position.position.is_game_over()
        ):
    ...
    def ambiguous(self):
        if self.best_evaluvation is not None and self.next_best_evaluvation is not None:
            if self.best_evaluvation.is_mate() and self.next_best_evaluvation.is_mate():
                if (
                    self.best_evaluvation.mate() > -1
                    and self.next_best_evaluvation.mate() > -1
                ):
                    return True
            elif self.best_evaluvation.is_mate() and not (
                self.next_best_evaluvation.is_mate()
            ):
                if self.next_best_evaluvation.cp > 200:
                    return True

            elif not (self.best_evaluvation.is_mate()) and not (
                self.next_best_evaluvation.is_mate()
            ):
                if self.best_evaluvation.cp < 210 or self.next_best_evaluvation.cp > 90:
                    return True
        return False
    ...
    def material_difference(self):
        return sum(
            v
            * (
                len(self.position.pieces(pt, True))
                - len(self.position.pieces(pt, False))
            )
    ...
    def generate(self):
        ...
        logging.debug(
            log_colors.print_blue("Material Value: " + str(self.material_difference()))
        )
        ...
        if self.best_move and not self.ambiguous() and not self.game_over():
            ...
            logging.debug(
                log_colors.print_green("   Ambiguous: " + str(self.ambiguous()))
            )
            logging.debug(
                log_colors.print_green("   Game Over: " + str(self.game_over()))
            )
            logging.debug(
                log_colors.print_green(
                    "   Has Best Move: " + str(self.best_move != None)
                )
            )
            ...
            logging.debug(
                log_colors.print_warning("   Ambiguous: " + str(self.ambiguous()))
            )
            logging.debug(
                log_colors.print_warning("   Game Over: " + str(self.game_over()))
            )
            logging.debug(
                log_colors.print_warning(
                    "   Has Best Move: " + str(self.best_move != None)
                )
            )
    ...
    def is_complete(self, category, color, first_node, first_val):
        if self.next_position is not None:
            if (category == "Mate" and not self.ambiguous()) or (
                category == "Material" and self.next_position.next_position is not None
            ):
    ...
    def evaluate_position(self):
        if self.player_turn:
            info = self.engine.analyse(
                self.position, chess.engine.Limit(depth=self.depth), multipv=2
            )
            ...
            info = self.engine.analyse(
                self.position, chess.engine.Limit(depth=self.depth)
            )
            self.best_move = info.pv[0]
            self.best_evaluvation = info.score.relative
            self.next_best_evaluvation = None
        if self.best_move is not None:
            self.next_position = PositionTest(
            ...
            logging.debug(
                log_colors.print_green(
                    "Best Move: " + self.next_position.position.san(self.best_move)
                )
            )
            self.next_position.position.push(self.best_move)
            logging.debug(
                log_colors.print_blue(
                    "   CP: "
                    + (
                        str(self.best_evaluvation.cp)
                        if not (self.best_evaluvation.is_mate())
                        else "None"
                    )
                )
            )
            logging.debug(
                log_colors.print_blue(
                    "   Mate: " + str(self.best_evaluvation.is_mate())
                )
            )
            if self.next_best_evaluvation != None:
                ...
                logging.debug(
                    log_colors.print_blue(
                        "   CP: "
                        + (
                            str(self.next_best_evaluvation.cp)
                            if not (self.next_best_evaluvation.is_mate())
                            else "None"
                        )
                    )
                )
                logging.debug(
                    log_colors.print_blue(
                        "   Mate: " + str(self.next_best_evaluvation.is_mate())
                    )
                )
            return True
        else:
